...
def generate_relative_linear_trajectory_3d(X0, Y0, Z0, length_m: float = 0.1, num_points: int = 50, direction_rad: float = 0.0):
    ...
    dx = length_m * math.sin(direction_rad)
    dy = length_m * math.cos(direction_rad)
    X1 = X0 + dx
    Y1 = Y0 + dy
...
def generate_curved_trajectory_3d(
    ...
    if turn_left:
        ...
        Xc = X0 + R * math.cos(psi0)
        Yc = Y0 - R * math.sin(psi0)
        # vector from center to points (rotating CCW by +phi)
        Xs = Xc - R * np.cos(psi0 + phi)
        Ys = Yc + R * np.sin(psi0 + phi)
...
def generate_sine_trajectory_3d(
    ...
    Ys = np.linspace(Y0, Y0 + length, num_points)
    Zs = np.full(num_points, Z0, dtype=np.float64)

    # sinusoidal Y
    Xs = X0 + amplitude * np.sin(2*np.pi*(Ys - Y0) / wavelength)

    # derivative dY/dX = (2π / wavelength) * amplitude * cos(...)
    dYdX = (2*np.pi / wavelength) * amplitude * np.cos(2*np.pi*(Ys - Y0) / wavelength)
